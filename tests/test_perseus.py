...
def test_check_perseus_treats_successful_query_probe_as_available(monkeypatch, tmp_path):
    ...
    assert "query probe works" in rendered


def test_check_perseus_falls_back_to_local_query_probe(monkeypatch, tmp_path):
    calls = []
    ...
    def fake_run(command, cwd, text, capture_output, timeout, check):
        ...
        if tuple(command) == ("/bin/perseus", "index", "--status"):
            return SimpleNamespace(returncode=2, stdout="", stderr="no ready index")
        if tuple(command) == ("/bin/perseus", "query", "where is code?"):
            return SimpleNamespace(returncode=2, stdout="", stderr="cannot reach perseus API")
        if tuple(command) == (
            "/bin/perseus",
            "query",
            "--local",
    ...
    assert report.ok
    assert report.query_available
    assert report.query_mode == "local"
    assert calls == [
        ("/bin/perseus", "--version"),
        ("/bin/perseus", "index", "--status"),
        ("/bin/perseus", "query", "where is code?"),
        ("/bin/perseus", "query", "--local", "--files-only", "where is code?"),
    ]

    rendered = format_perseus_report(report)
    assert "local_query_probe: ok" in rendered
    assert "via local query" in rendered
